<template>
    <v-container class="text-sm-center abn-width-fix">
        <v-card>
            <v-card-text>
                <v-container grid-list-md>
                    ...
                    <v-form v-model="valid">
                        <v-text-field
                                v-on:keyup.enter="$refs.email_field.focus()"
                        ...
                        <v-text-field
                                ref="password_field"
                                v-on:keyup.enter="sendRegistration"
            ...
            <v-card-actions>
                <v-spacer></v-spacer>
                <v-btn color="orange darken-2" flat @click.stop="sendRegistration" :disabled="isDisabled">
                    Зарегестрироваться
                </v-btn>
            </v-card-actions>
        </v-card>
        ...
        data() {
            return {
                ...
                nickname: {
                    value: '',
                    rules: [
                        v => !!v || 'Никнэйм обязателен',
                        v => v.length <= 100 || 'Никнэйм не может быть более ста символов',
                        v => v.length >= 6 || 'Никнэйм не может быть менее шести символов',
                    ],
                },
                email: {
                    value: '',
